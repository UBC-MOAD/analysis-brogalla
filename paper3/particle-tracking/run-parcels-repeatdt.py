...
def run_parcels(fieldset, plon, plat, pdepth, runtime=0, sampling=False, filename='test.zarr'):
    ...
    pset = ParticleSet(fieldset=fieldset, pclass=PbParticle, lon=plon, lat=plat, depth=pdepth, 
                       repeatdt=delta(hours=24*7*4).total_seconds()) # release every 12 weeks
...
pset = run_parcels(fieldset, PC_lons, PC_lats, PC_deps, runtime=delta(hours=24*30*36), 
        sampling=True, filename=f'run-{year}-{month:02}-repeatdt-36months_20230325.zarr') 
